...
from apis import sf_backends, nb_backends
from . import models
...
def update(request):
    ...
    try:
        max_time = timezone.now() - timezone.timedelta(
            minutes=1)  # the newest selected field must be at least 1 minutes old which is the time that salesforce need
        # print timezone.now()
        # print max_time
...
def save_update(request):
    if request.method == "POST":
        ...
        try:
            matching_contacts = models.ContactSync.objects.filter(email=content['payload']['person']['email']).update(
                contact=request.body, synced=False)
        except models.ContactSync.DoesNotExist:
            matching_contacts = None
